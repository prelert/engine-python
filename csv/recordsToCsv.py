...
############################################################################
"""
Example of how records can be queried from the Engine API in CSV format
using Python 2.6+ (including Python 3.x).  No extra modules are required beyond
those that come with a base Python install.

Usage:

python recordsToCsv.py <job> <server_hostname> [ <server_port> [ <result_limit> ] ]
...
Records are returned in descending order of normalized probability; the most
unusual will be at the top of the list.
"""
...
url = 'http://%s:%s/engine/v1/results/%s/records?take=%s&sort=normalizedProbability' % (server, port, job, limit)
